...
100 dpi once scaled to the page width. This script fixes both, rather than `l63/plots.py`,
...
Five figures are written; three are placed on the sheet. None exists anywhere else:
  ...
  p_hook         the opening figure: the butterfly in grey with one MLP rollout in red over
                 ...
                 do not. The window is 223 steps; `hook` gives the constraints it satisfies.
  p_system       the Lorenz distance between two true trajectories 0.01 apart, median over
                 ...
                 lambda_1. Written but not placed; nothing else on the sheet shows sensitive
                 dependence directly, and placing it is one line in poster_final.typ.
  p_conditional  the two ground truths drawn rather than tabulated: one time step taken 500
                 times from the same state. A point on the ODE, a cloud on the SDE, whose
                 width is the experiment's control parameter.
  ...
  p_scorecard    §4. The four rulers as dots with the five-seed range drawn through them, in
                 place of two 30-cell tables. Several of these columns are bimodal, so the
                 range is what says whether two models are distinguishable.
...
# REPRESENTATIVE carries seven: adding a rung upstream must not add a panel to a figure laid
# out for five.
# Keys are row ids, the sheet's five slots in order; values are the `topic_label` stem of the
# row's key in summary.json. The stem is written out rather than rebuilt from the row id,
# because in the AR set below two rows are both topic 03 (the MLP at two k).
...
# is not what `poster_final.typ` places; it is kept so the comparison can be redrawn from the
# AR checkpoints, which exist.
#
# Row 02 stays the one-step MLP and row 03 is the same network at k = 4, so the pair still
# measures what training through the composed map does. Replacing the baseline instead of
# pairing with it would leave nothing for the other rows to improve on.
#
...
# log-sigma to the floor and makes the model deterministic; an AR-NLL makes a one-step sigma
# head carry k-step uncertainty and then injects it at every step. Per-step training is the
# standard choice for a probabilistic forecaster (GenCast).
...
# the opening figure shows the baseline the rest of the sheet is read against. IC 47 is the
# 54th percentile of that model's 128 starts. The IC moves with the model and with the window,
# so re-pick it with `pick_ic.py` when either changes rather than carrying one across.
...
# Drawn width in inches, and the smallest type each figure is allowed. Both follow from the
# width the figure is placed at in poster.typ, which sets the scale factor and so the printed
# size of every label:
#
...
#
# The how-to deck's floor is 18 pt inside a figure, and no size here falls under it once
# scaled, which is why the small text below is 10.6 pt rather than the 8 pt a screen figure
# would use. If a figure is placed at a different width, these sizes move with it.
...
# Drawn at W1 it would scale 2.57x and its labels would read at 28 pt, making the opening
# figure's furniture the largest type in any figure on the sheet.
WH = 8.6
...
def rep(topic: str, kind: str) -> int:
    """The seed a figure draws: the one whose horizon is nearest the median.

    `run/report.py` picks it, records it as `rep_seed`, draws its own figures from it, and puts
    that seed's numbers in `horizon_detail`. Five of the ten rep_seeds are not 0, so drawing
    seed 0 here would plot one seed's curve under another seed's horizon marker.
    """
...
def hook(steps: int = 223, ic: int | None = None, topic: str | None = None,
    ...
    rollout from the same initial state, layered on top. For most of the window the red sits
    under the grey; then it takes a loop the truth does not take, and from there the two lie
    on the same attractor along different paths.

    `steps` is set in Lyapunov times: 223 steps is 5.00 tau at dt = 0.025 and
    lambda_1 = 0.897. Tau is the unit §1 prints beside the step convention, and it carries
    across to results outside this project.

    The window trades two constraints against each other. The butterfly needs enough steps to
    be drawn at all, and the divergence needs few enough that the two curves have not both
    covered the whole attractor. Past about 15 tau each curve traces the full shape and no
    disagreement is visible. At 5 tau the shape is drawn about seven times round and the
    forecast is 79 steps past its own crossing.

    The window and the IC go together. Two of the filters that pick the IC, the lobe-change
    disagreement and the gap at the last drawn step, are measured inside the window and do not
    survive a change to it. At 9.0 tau this figure used IC 113; at 5.0 tau the same IC leaves
    the two curves 4.3 apart at the end, because they pass close to each other there.

    No step count is printed on this figure. A number here would be either a different
    statistic from §4's horizon or a single draw; §4 carries the measurement with its seed
    range.

    Both figure banks draw the one-step MLP (`HOOK_TOPIC_AR` is '02'), so `p_hook` and
    `p_hook_ar` are the same figure: the baseline is the row the rest of the sheet is read
    against.

    IC 47 sits mid-distribution: its error reaches the attractor scale at 156 steps against
    this model's median of 148 over the 128 evaluation starts, the 54th percentile, where the
    crossing runs from 35 to 901 and 8 never cross inside 900 steps.

    Legibility is a second filter. The truth makes 5 lobe changes inside the 223-step window
    and the forecast makes 4, so the red covers the butterfly while taking its own path, and
    the two are 24.0 apart at the last drawn step, past the attractor's own 14.0. An IC that
    makes the same lobe changes as the truth keeps the red under the grey for nearly the whole
    window. IC 47 is also not captured: its 20 000-step rollout has a spread of 14.6 over its
    final 5000 steps against truth's ~14.4.

    Re-pick from the model rather than copying an IC across when row 02 or the window changes.
    ...
    156 and §4's 336 are different statistics: `evaluate.horizon` crosses the attractor scale
    with the median error curve over 128 ICs (`median_curve`) rather than with a single
    rollout, and the median curve crosses later than the median single rollout does (336
    against 148).
    ...
    # butterfly's proportions for an axis whose range is the range of the data. Do not copy it
    # into a figure whose subject is the shape.
    #
    # The z limits come from both curves. Framing on the truth alone would clip the model's
    # excursion, which is what the figure shows.
    zlo = float(min(truth[:, 2].min(), pred[:, 2].min()))
    zhi = float(max(truth[:, 2].max(), pred[:, 2].max()))
    zpad = 0.08 * (zhi - zlo)

    # x ticks at +-20, x limits at +-22. The limits are two units wider than the outermost tick
    # so that the "-20" and "20" labels sit inside the frame with air around them, the way the
    # z labels do: z's ticks land on round decades (10 to 40) inside limits of roughly 3 to 48,
    # so they are never near a corner.
    #
    ...
    with plt.rc_context({'font.size': F1, 'axes.titlesize': F1, 'axes.labelsize': F1,
        ...
        # The drawn aspect matches the placed aspect: 365 mm wide by 235 mm tall on the sheet,
        # so 8.6 in by 8.6 * 235/365 = 5.53. The rest follows from `adjustable='datalim'`.
        fig, ax = plt.subplots(figsize=(WH, 5.53))

        # Truth thick and underneath, forecast thinner on top, so where the two agree the grey
        # shows as a halo around the red instead of one colour hiding the other.
        ...
        # A legend rather than labels on the curves: for most of the window the two coincide,
        # so there is nowhere on the red that is not also the grey.
        ...
        # No `set_aspect`: the axes fill the panel and the data is stretched to fit it. The
        # frame block above gives the reason and the cost.
        ax.set_xlim(*frame[0])
        ax.set_ylim(*frame[1])
        ...
        # printed -16 and 16 as the outermost labels on an axis running to +-20. The limits are
        # pinned to the range of the data, so the labels state that range.
...
def system(steps: int = 600, delta: float = 1e-2, n_ic: int = 128, seed: int = 0) -> None:
    ...
    Median over 128 pairs. A single pair does not give a clean exponential: the eval set's
    IC 0 spends its first 400 steps spiralling near C-, where local expansion is weak, so its
    gap contracts from 0.010 to 0.005 before it grows and its average rate reads 0.58 rather
    than 0.897. Over 128 pairs the median fits 0.851 per time unit against a measured
    lambda_1 of 0.897, within 5 %, so the straight stretch on the log axis carries the Lyapunov
    exponent and the reference line is drawn through it.
    ...
    for kind in KINDS:
        ...
        # moves every run; p_attractors seeds the Gaussian for the same reason.
    ...
    with plt.rc_context({'font.size': F1, 'axes.titlesize': F1, 'axes.labelsize': F1,
        ...
        # The slope is lambda_1. Anchored on the ODE median inside the fitted stretch (steps
        # 100-350) and drawn only across it, so it is not extrapolated into the initial
        # transient or into saturation, where it does not hold.
        ...
        # Labelled below the line rather than at its end: at its end it sits between the two
        # dotted ceilings and reads as belonging to one of them.
...
def _spread_anchors(states, k: int):
    ...
    Two rejected alternatives, both tried:
      ...
      * farthest-point sampling. It separates the anchors fully and picks the wing tips, where
        the flow is fastest and the conditional widest: the 27.7 %-of-scale end of a
        distribution whose other end is 9.0 %. Three extremal anchors make the SDE cloud look
        uniformly wide, and they push the shared axes out far enough to shrink the ODE
        butterfly beside them.

    Quantiles of x rather than of the trajectory index for the same reason as the first bullet:
    the figure is drawn in x-z, so the anchors are spread in that plane.
    """
    x = states[:, 0]
...
def conditional(n_samples: int = 500, n_anchor: int = 3, n_show: int = 800,
    ...
    seven the panel is a dense grey tangle with the red marks overlapping each other; at three
    each anchor stays separable as a dot in the left panel and a smear in the right.
    ...
    straight edges. Those edges come from the display sampling rather than the geometry: the
    path is continuous and rough because of the multiplicative noise, which all 100 substeps
    per stored step show. The models still see only the stored states; this changes the
    rendering alone. The display trajectory is integrated fresh from the frozen reference's
    ...
    Measured content:
      * ODE. The spread is 0.000 in all three coordinates at every anchor, exactly zero,
        because 500 deterministic integrations from one state give one state. Each anchor is
        a point.
    ...
    conditional's width depends on position on the attractor. Three anchors show less of that
    variation than seven would.

    The figure does not separate noise from motion. At some anchors one step moves the state
    further than the cloud is wide (11.6 against a radius of 4.4) and at others the reverse
    (0.71 against 2.2); both are drawn identically.

    `_spread_anchors` chooses the anchors deterministically, so the picture is the same every
    run. They are not evenly spaced along the trajectory: even spacing in time puts them
    wherever the trajectory happens to be, and on the SDE at three anchors that put all three
    on the same wing, where the clouds merged into one blob.
    """
    with plt.rc_context({'font.size': F1, 'axes.titlesize': F1 + 1,
        ...
        #
        # These panels are not equal-aspect, and the butterfly above them is. A unit of x is
        # about 1.4x longer than a unit of z here, so both attractors are drawn wider and
        # flatter than they are. The constraint is page height: equal aspect at 365 mm wide
        # needs about 215 mm against the 170 mm this uses, and §1 does not have the extra
        # 45 mm. What this figure shows is the width of the conditional; the hook shows the
        # shape and is proportioned for it. Do not copy this compromise into a shape figure.
        fig, axes = plt.subplots(1, 2, figsize=(WH, 4.0), sharex=True, sharey=True)

        for ax, kind in zip(axes, KINDS):
            b = GT['datasets'][kind]['b']
            d = data(kind)
            # Substep resolution for the display path; see the docstring. n_show stored steps,
            # all 100 substeps of each kept, from the frozen ref's first state.
...
def attractors(n_show: int = 6000, n_ic: int = 4, steps: int = 8000) -> None:
    ...
    Two rows, one per ground truth, sharing axes within a row, so every panel in a row is at
    one scale and the comparison across the row is direct.

    An earlier version put each model's forecast-error curve in a row under its attractor,
    removed because two of the four rows were four flat lines at a ceiling, and `horizon` is
    in the scorecard with its five-seed range, which a single curve cannot show.

    Drawn for a two-column placement rather than the full width. Square panels and full width
    do not fit at this row count: six panels across 751 mm are 120 mm wide each, so two square
    rows cannot be shorter than about 330 mm. Placed at 494 mm the same figure is 218 mm, the
    height of the scorecard beside it. The type is sized for 494 mm, so moving the placement
    means moving the sizes, and each axis is held to three ticks, because five tick labels do
    not fit a 41 mm panel.
    """
    cols = ['truth'] + [SHORT[t] for t in MODELS]

    # Square panels, both rows the same size. A butterfly drawn with a unit of x wider than a
    # unit of z is a different shape, so `set_aspect('equal')` has to stay. The two truths do
    # not have the same proportions (the ODE spans about 40 in x and 46 in z, the SDE 54 and
    # 72), so framing each on its own data gives two rows of different height. Widening the
    # shorter axis of each frame until its two spans match makes every panel a square of the
    # same size. The cost is air: the SDE frame goes out to +-36 in x where its data reaches
    # +-27, so the SDE attractors sit smaller inside their squares.
    frames = {}
    for kind in KINDS:
        ...
        # The two truths do not have the same proportions (the ODE spans about 40 in x and 46
        # in z, the SDE 54 and 72), so framing each on its own data gives two rows of different
        # height. Widening the shorter axis of each frame until the two spans match makes every
        # panel a square of the same size, at the cost of air around the SDE, and
    ...
    with plt.rc_context({'font.size': F2, 'axes.titlesize': F2, 'axes.labelsize': F2,
        ...
        for r, kind in enumerate(KINDS):
            ...
            # Each row is framed on its own truth, with 10 % of air. One frame shared across
            # both rows spends 40 % of the ODE row's height on a band the ODE never visits, and
            # the comparison inside a row is truth against model. A model that leaves the
            # frame, the Gaussian, which over-disperses, is drawn running off the top edge.
...
# printed beside it, so the column stays readable and the value is still shown.
...
def scorecard() -> None:
    ...
    `horizon`, `climate` and `alive` are bimodal here, and where two bars overlap the two
    models are not distinguishable by this experiment. The alternative is two thirty-cell
    tables: 150 printed numbers against this figure's 50, and no view of the overlap.

    The row count is `len(MODELS)`; the figure box is fixed. Rows are laid out on a y axis
    running from -0.55 to `n + 0.80` inside a fixed 4.15 in height, so adding a model tightens
    the row pitch and changes nothing else: the drawn width, the scale factor at which
    `poster.typ` places it, and every type size in the table stay put. At n = 6 the pitch is
    about 41 pt against 10.6 pt labels, so a sixth row costs pitch and nothing else.
    """
    topics = list(MODELS)
    n = len(topics)
    ...
    with plt.rc_context({'font.size': F3, 'axes.titlesize': F3, 'axes.labelsize': F3,
        ...
        for r, kind in enumerate(KINDS):
            ...
            for c, (key, head) in enumerate(RULERS):
                ...
                # Every reference is drawn, named and given its value, in a band above the five
                # models. It is what the column is read against and it differs between the two
                # datasets, so the label carries both the name and the number.
                drawn_fracs = []
...
def _mark(ax, y, v, x0, x1, fmt, log) -> None:
    ...
    if med <= x0:
        ...
        # Print to the right of the dot, or to its left where the dot is far enough right that
        # the label would run off the axis.
...
if __name__ == '__main__':
    ...
    # hand-picked figure (the hook, with its chosen IC) from being rebuilt along with the rest.
    ...
    if a.ar:
        ...
        #
        # `p_hook` is drawn, from row 02's AR model and its own initial condition, which is why
        # HOOK_IC moves with MODELS. 122 is the 6th percentile for the k=4 rollout model and
        # the 69th for the one-step MLP, so carrying one IC across the two banks would put a
        # near-worst rollout on the opening figure. See the second warning in `hook`.
