...
plus `alive`, the fraction of long rollouts still moving like the system. The brief names it
as one of its two tests, so it is reported alongside the four rulers.

Read as: how long is it right; is its uncertainty calibrated; does it live in the right
place; does it move the right way; is it still moving at all.

No level here is tuned against a model. Every level a number is read against is measured from
...
dataset. Two constants are chosen: `alive_frac`'s 0.25 / 5000 (see its docstring for the
sensitivity) and `vpt`'s 0.4 (the published NRMSE convention, quoted so outside numbers are
comparable, and never mixed with the step counts).
...
def rollout_direct(model, xs: Tensor, std: Tensor) -> tuple[Tensor, Tensor]:
    ...
    call, against the same network asked for it as s repeated single steps. The two make
    different mistakes (one approximation of a hard map, or s approximations of an easy map
    compounding), and which wins says where the long-horizon error comes from.
    """
    model.eval()
    s = model.s_max
    with torch.no_grad():
...
def ensemble(model: ForecastModel, x0: Tensor, steps: int, n_samples: int) -> Tensor:
    ...
    A deterministic model returns the same forecast every call, which is why sampling is not
    baked into `forecast`: the probabilistic rungs get their spread from repeated calls.
    """
    model.eval()
    with torch.no_grad():
...
def median_curve(e: Tensor) -> Tensor:
    ...
    Uses quantile(0.5) rather than median(): with an even number of ICs torch.median returns
    the lower of the two middle values, which near the flat ceiling shifts the crossing by
    tens of steps. A median rather than a mean, because chaos makes a few ICs saturate far
    earlier than the rest and a mean would track those instead of the typical run.
    """
...
def horizon(curve: Tensor, floor: Tensor, bar: Tensor, scale: float) -> dict:
    ...
    are directly comparable and answer "how many steps can be taken" on one axis:
      ...
      euler_bar_steps   the same, for one explicit-Euler step of dt per time step, a
                        classical solver of the same cost as one model call.
      floor_steps       the same, for the gap between Euler at dt_int and at dt_int/2. Past
                        here "tracks the truth" can only mean "tracks this project's
                        numerical ground truth", which bounds what any claim here can say.

    Two crossings, which say where the model sits between those references:

      beyond_noise      the model leaves the ground truth's own uncertainty. Typically n ~ 1,
                        which says the model is not a drop-in for the integrator at any
                        horizon.
      loses_to_euler    where the model first becomes worse than the same-cost solver, or -1.
                        ...
                        both have saturated the crossing is an artefact of two flat lines,
                        and this returns -1 instead of a step count.
    """
...
def vpt(err: Tensor, scale: float, dt: float, lam: float = 0.906,
    ...
    Reported so the result can be compared with published numbers. It uses a stricter bar
    than the step counts above, so the two are not interchangeable and are never printed
    side by side.
    """
...
def truth_spread(d: Data, n_ic: int = 32, steps: int = 200,
    ...
    On the SDE two realisations from the same state differ, because the noise is part of the
    system, so truth has a real ensemble spread and "how uncertain should a forecast be here"
    has an answer measured from the data.

    The lead time is measured too: it is the step at which truth's spread first reaches half
    its saturated value, where there is most to distinguish. This function computes it once
    per dataset and every model is read at that same lead time, both because it is the
    expensive part and because a shared lead time is what makes the column comparable.

    Returns (nan curve, -1) on the ODE, where truth's ensemble spread is identically zero.

    Seeded, because this curve is the denominator of every model's `spread` and must not move
    between runs. Pass a different `seed` to draw a second, independent truth ensemble;
    scoring that one against the first measures the ruler's own noise at this ensemble size.
    """
    if d.kind == 'ode':
...
def w1(a: Tensor, b: Tensor, n_q: int = 2000) -> float:
    ...
    the two quantile functions. Evaluating both on a shared grid of probabilities keeps it
    correct for samples of different sizes: comparing sorted values elementwise only works
    when the two samples are the same length, and silently compares the wrong parts of the
    two distributions when they are not.
...
def _segments(x: Tensor, seg: int) -> Tensor:
    ...
    same footing. W1's sampling error is set by the number of independent pieces rather than
    by the number of states, so 32 rollouts of 20 000 steps and 64 trajectories of 901 are not
...
def climate_ratio(model_states: Tensor, truth_states: Tensor, n_splits: int = 20,
    ...
    true sample. That turns an absolute distance into a ratio: 1.0 is as close as two samples
    of the true system are to each other, which makes it the floor rather than a target.

    `truth_states` must keep its trajectory axis, shape (n_ic, T, 3), and must be long true
    rollouts of the same shape as the model's: pass what `ground_truth.truth_rollouts`
    generates, not the evaluation set and not the single reference trajectory. Point 3 below
    is why the 901-step evaluation trajectories do not serve.

    Three things have to match, or the ruler measures sampling instead of distribution:

      1. Split the truth by trajectory rather than by state. States along one trajectory are
         strongly correlated; a random split compares a pool against itself and makes the
         denominator far too small.
      2. Match the number of independent pieces rather than the number of states. W1's
         sampling error is set by the former. Thinning a model's 32 x 20 000 rollout and
         truth's 64 x 901 to the same state count leaves the model with ~11x the effective
         sample: measured, an independent draw of the truth scored 0.565 at 128 trajectories,
         0.665 at 64 and 0.972 at 16, so most of the "score" was sample size.
      3. Match the length, because 901 steps is only 20 Lyapunov times and that is not
         ...
         sampling the attractor well: truth in the model's own shape scored 1.69.

    So this function takes both sides as long rollouts, cuts them to a common length, and
    draws the same number of pieces from each. With the defaults that is 32 pieces of 20 000
    steps per side.
    ...
    Ratio of means rather than mean of ratios: a ratio of two noisy quantities is biased
    upward.

    WHERE TRUTH LANDS. The denominator is two disjoint halves of one finite pool, so an
    outlier in one half is necessarily absent from the other and the two are pushed slightly
    apart; an independent draw is under no such constraint. The anchor is therefore the
    measured `truth_climate` in ground_truth.json, and `climate_vs_truth` divides by it;
    that ratio cancels the effect, since it acts identically on truth and on every model.
    Read a model against the truth row, never against 1.
    ...
    does not resolve fine distributional differences.
...
def chaos_ratio(model: ForecastModel, d: Data, lam_true: float,
    ...
    The same Benettin estimator, the same time step and the same number of steps; only the
    map differs, so the ratio compares like with like rather than against a literature
    ...
    dynamics. So for a sampled model this column describes the mean/zero-base map rather than
    the map the other rulers roll out, and it is read as such. It is also why `mean_only`
    carries no `chaos`: it would be this same number by construction, so the two could never
    differ.

    The map is the model's own, via `map_state` / `map_step`: whatever object `forecast`
    iterates is what gets measured, the window for a Markov model and (u, h, c) for a
    ...
    damps the sensitivity that defines the system. At or below 0 the model is not chaotic at
    all: it has found a fixed point or a limit cycle.
...
def alive_frac(states: Tensor, truth_std: float, window: int = 5000,
    ...
    truth's, meaning it has fallen into a fixed point or a tight cycle. This measures the
    final window only: a trailing-window criterion has no false "recovered" state and needs
    no persistence tuning, unlike a probe that slides along the trajectory.

    `ratio` and `window` are chosen numbers, the only two in the ruler suite, and neither is
    tuned against a model. The quantity is bimodal (a rollout keeps most of the attractor's
    spread or almost none of it), so anything between roughly 0.05 and 0.6 gives the same
    partition, and the truth row goes through this same function as the check. The ruler is
    insensitive to those two constants rather than free of them, and the truth row is printed
    beside it.
...
def lobe_frac(states: Tensor, dt: float, lam: float = 0.906) -> Tensor:
    ...
    Spread-based collapse detection does not see a model that keeps moving at full amplitude
    inside one wing: a period-p orbit reads as alive. Counting sign changes of x catches
    that, and truth's own value is the reference.
    """
    sign = states[..., 0].sign()
